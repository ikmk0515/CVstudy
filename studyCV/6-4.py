...
class TrafficWeak(QMainWindow):
    ...
    def signFunction(self):
        self.label.clear()  # 레이블 지우기
        self.label.setText('교통약자 번호판을 등록합니다.') # 출력할 텍스트
        
        for fname,_ in self.signFiles:  # 표지판 영상 파일 하나씩 불러오기
            self.signImgs.append(cv.imread(fname))  
            cv.imshow(fname,self.signImgs[-1])     

    def roadFunction(self):
        if self.signImgs==[]: # 저장된 영상이 없다면 True
    ...
    def recognitionFunction(self):
        if self.roadImg is None:    
            ...
            for sign_kp,sign_des in KD: # 키포인트와 기술자 각각 대입하여 반복문 실행
                knn_match=matcher.knnMatch(sign_des,road_des,2) # sign_des 와 road_des 매칭. 특징점마다 최근접 이웃 2개 찾음
                T=0.7
                good_match=[]
            ...
            if len(GM[best])<4:   # 최선의 번호판이 매칭 쌍 4개 미만이면 실패
                ...
                self.label.setText(self.signFiles[best][1]+' 보호구역입니다. 30km로 서행하세요.')   # best 0:어린이, 1:노인, 2:장애인                 
                winsound.Beep(3000,500) # 삡 소리 실행
